...
class FacultyProfile(BaseModel):
    ...
    class Meta:
        db_name = "faculty_profile"
        verbose_name = "Faculty Profile"
        verbose_name_plural = "Faculty Profiles"
        
    def clean(self):
        if self.user and self.user.role != Roles.FACULTY:
            raise ValidationError(f"User with email {self.user.email} is not a faculty.")
...
class StudentProfile(BaseModel):
    ...
    def clean(self):
        if not self.user or self.user.role != Roles.STUDENT:
            raise ValidationError(f"User with the email {self.user.email} is not a student")
